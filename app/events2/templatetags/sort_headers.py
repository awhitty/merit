...
class SortHeaders:
    ...
    def __init__(self, request, headers, default_order_field=None,
        ...
        if ORDER_TYPE_VAR in params and params[ORDER_TYPE_VAR] in ('desc', 'asc'):
    ...
    def headers(self):
        ...
        for i, (header, order_criterion) in enumerate(self.header_defs):
            ...
            if i == self.order_field:
                ...
                new_order_type = {'desc': 'asc', 'asc': 'desc'}[self.order_type]
            yield {
                'text': header,
                'sortable': order_criterion is not None,
                'url': mark_safe(self.get_query_string({ORDER_VAR: i, ORDER_TYPE_VAR: new_order_type})),
                'class_attr': mark_safe(th_classes and ' class="%s"' % ' '.join(th_classes) or ''),
            }
...
register = template.Library()
register.inclusion_tag('includes/table_header.html', takes_context=True)(table_header)